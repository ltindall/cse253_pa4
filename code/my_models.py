...
def train(model, optimizer, epochs, train_inputs, sequence_len, hidden0): 
    # get a list of start indices based on the inputs
    pm_list = utils.permute_list(len(train_inputs), sequence_len+1, overlap=False)
    ...
    for i in range(epochs): 
        ...
        while True: 
            # create minibatch
            batch, ids = utils.create_batch(training_inputs, pm_list, batch_size, sequence_len)
            ...
            end_id = start_id+sequence_len

            # split batch into inputs and targets
            inputs = Variable(torch.LongTensor(train_inputs[start_id:end_id])).view(sequence_len, -1)
